...
namespace RayCarrot.Rayman
{
    ...
    /// </summary>
    public class UbiArtIPKFile : IBinarySerializable
    {
        #region Constructor
        ...
        public UbiArtIPKFile(UbiArtSettings settings)
        {
            Settings = settings;
        ...
        /// <summary>
        /// The file extension for cooked files
        /// </summary>
        public const string CookedExtension = ".ckd";
        ...
        public UbiArtTEXFile GetTEXFile(Stream ipkFileStream, uint baseOffset)
        {
            // Get the decompressed bytes and load them into a memory stream
            using var stream = new MemoryStream(GetFileBytes(ipkFileStream, baseOffset));

            // Return the data
            return GetTEXFile(stream);
        }
        ...
        /// <returns>The texture</returns>
        public UbiArtTEXFile GetTEXFile(Stream fileStream)
        {
            // Serialize the data
            var data = new UbiArtTextureSerializer(Settings).Deserialize(fileStream);
        ...
        public byte[] GetFileBytes(Stream fileStream, uint baseOffset, bool decompress = true)
        {
            // Set the position
        ...
        public void Deserialize(BinaryDataReader reader)
        {
            ...
            if (IPKVersion >= 5)
            {
                ...
                if (Settings.Game == UbiArtGame.JustDance2017)
                {
                    // Read the path
