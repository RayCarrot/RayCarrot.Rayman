...
{
    /// <summary>
    /// The wrapper binary data reader for the <see cref="BinaryDataSerializer{T,Settings}"/>
    /// </summary>
    public class BinaryDataReader<Settings> : IBinaryDataReader<Settings> 
        where Settings : BinarySerializerSettings
    {
        ...
        public BinaryDataReader(BinaryReader reader, Settings serializerSettings)
        {
            Reader = reader;
            ...
            object ReadObject()
            {
                ...
                switch (Type.GetTypeCode(type))
                {
                    case TypeCode.Object:
                        ...
                        if (!typeof(IBinarySerializable<Settings>).IsAssignableFrom(type))
                            throw new NotSupportedException($"The specified generic type does not implement {nameof(IBinarySerializable<Settings>)}");

                        // Create a new instance
                        var instance = type.CreateInstance<IBinarySerializable<Settings>>();
